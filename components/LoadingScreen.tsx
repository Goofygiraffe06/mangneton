...
export const LoadingScreen: React.FC<LoadingScreenProps> = ({
  ...
  const calculateTotalProgress = (items: ProgressItem[]) => {
    if (items.length === 0) return 0;
    const total = items.reduce((acc, item) => acc + item.progress, 0);
    // Rough estimate: Assume ~5 files per model for avg calculation
    return Math.min(Math.round(total / Math.max(items.length, 1)), 100);
  };

  const embedderTotal = calculateTotalProgress(embedderProgress);
  const generatorTotal = calculateTotalProgress(generatorProgress);
  
  // Calculate global progress roughly (Embedding is small, Generator is large)
  // Let's weight generator 80% and embedder 20%
  const globalProgress = Math.round((embedderTotal * 0.2) + (generatorTotal * 0.8));
        ...
        <div className="bg-gray-800 rounded-xl p-6 shadow-xl border border-gray-700 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 pt-4 border-t border-gray-700/50">
            ...
            <div className="flex items-center space-x-3">
              <div className={`p-2 rounded-lg ${embedderTotal === 100 ? 'bg-green-500/10 text-green-400' : 'bg-gray-700 text-gray-400'}`}>
                <Database className="w-4 h-4" />
              </div>
              <div className="flex-1 space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="font-medium text-gray-300">Embedding Model</span>
                  <span className="text-gray-500">{embedderTotal === 100 ? 'Ready' : `${embedderTotal}%`}</span>
                </div>
                {embedderTotal < 100 && (
                  <div className="h-1 w-full bg-gray-700 rounded-full overflow-hidden">
                    <div className="h-full bg-blue-500 transition-all duration-300" style={{ width: `${embedderTotal}%` }} />
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Generator Status */}
            <div className="flex items-center space-x-3">
              <div className={`p-2 rounded-lg ${generatorTotal === 100 ? 'bg-green-500/10 text-green-400' : 'bg-gray-700 text-gray-400'}`}>
                <Cpu className="w-4 h-4" />
              </div>
              <div className="flex-1 space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="font-medium text-gray-300">Generation Model (LLM)</span>
                  <span className="text-gray-500">{generatorTotal === 100 ? 'Ready' : `${generatorTotal}%`}</span>
                </div>
                 {generatorTotal < 100 && (
                  <div className="h-1 w-full bg-gray-700 rounded-full overflow-hidden">
                    <div className="h-full bg-purple-500 transition-all duration-300" style={{ width: `${generatorTotal}%` }} />
                  </div>
                )}
              </div>
            </div>
